...
import { ArrowLeft, Check, Clock, BookOpen } from 'lucide-vue-next'
...
const quiz = computed(() => readingQuestions.find(q => q.id === quizId))
...
function submitQuiz() {
  ...
  if (quiz.value) {
    ...
    updateProgress('reading', score.value)
  }
          ...
          <div v-if="quiz" class="bg-white rounded-xl shadow-md overflow-hidden">
            <div class="p-6 bg-gray-50 border-b">
              <div class="flex items-center space-x-3">
                <BookOpen class="w-6 h-6 text-[#003366]" />
                <h2 class="text-xl font-bold text-[#003366]">{{ quiz.title }}</h2>
              </div>
              <p class="text-gray-500 mt-1">{{ quiz.description }}</p>
            </div>
            
            <div v-if="quiz.passage" class="p-6 border-b">
              <h3 class="text-sm font-medium text-gray-500 mb-3">阅读文章</h3>
              <div class="bg-gray-50 rounded-lg p-4">
                <p class="text-gray-700 leading-relaxed whitespace-pre-line">{{ quiz.passage }}</p>
              </div>
            </div>
